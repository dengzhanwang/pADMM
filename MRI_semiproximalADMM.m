...
while error > 1e-7 && iter < max_iter
    ...
    t_lin = semi_step_scale / (beta1 + beta2 + eps);
    Xnew = (X - t_lin * gradX) ./ (1 + rho);
...
tEnd = toc(tStart);
disp(['Semi-proximal ADMM Time: ', num2str(tEnd)]);

figure(1);
semilogy(e, 'LineWidth', 1.7, 'Color', 'r', 'Marker', '*');
xlabel('Iteration', 'FontSize', 16);
ylabel('Residual', 'FontSize', 16);
legend('Semi-proximal ADMM');
title('Semi-proximal ADMM Residual Convergence');
